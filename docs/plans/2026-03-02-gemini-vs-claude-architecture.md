...
Berdasarkan analisa source code Gemini CLI dan artikel
...
- Loop terjadi di **dua level**: internal (core, via `yield*` recursion) dan
  external (CLI, via `submitQuery` callback)
- Generator-based (`AsyncGenerator<ServerGeminiStreamEvent>`): events di-stream
  satu per satu
- Turn counter bounded (`MAX_TURNS = 100`)
- **Next-speaker check**: model terpisah (lightweight) menentukan apakah model
  utama perlu continue
...
- Model sendiri yang memutuskan kapan berhenti (via
...
> **NOTE**: Gemini CLI memisahkan **event production** (core) dari **event
> consumption** (CLI), memungkinkan multiple consumers (interactive mode,
> non-interactive pipe mode, Zed integration). Claude Code menggabungkan
> keduanya dalam satu loop.
...
- ~20+ built-in tools: `read-file`, `edit`, `write-file`, `shell`, `grep`,
  `glob`, `web-fetch`, `web-search`, `mcp-tool`, `ask-user`, `save_memory`, etc.
- Setiap tool memiliki `FunctionDeclaration` (JSON Schema) yang dikirim ke model
- **State machine** per tool call: Scheduled → Validating → AwaitingApproval →
  Executing → terminal
...
> **IMPORTANT**: Perbedaan filosofis besar: Claude Code percaya Bash sudah cukup
> sebagai "universal tool" — menghapus kebutuhan banyak tool wrapper. Gemini CLI
> memilih typed tools yang lebih terstruktur dan aman.
...
> **TIP**: Gemini CLI **tidak mempercayai model** untuk berhenti sendiri — ia
> menggunakan external detection. Claude Code **mempercayai model** sepenuhnya
> (model-driven autonomy). Ini mencerminkan perbedaan filosofi fundamental.
...
- Jika compression gagal (inflate token count), flag
  `hasFailedCompressionAttempt` mencegah retry
...
> **WARNING**: Perbedaan terbesar: Claude Code memiliki **sub-agents yang
> mengisolasi context** — task berat berjalan di context window terpisah dan
> hanya mengembalikan summary. Gemini CLI beroperasi dalam **single context
> window** dengan compression sebagai satu-satunya mekanisme.
...
- Dapat **stop execution**, **clear context**, atau **inject additional
  context**
...
- Hooks adalah **deterministic scripts** (bukan LLM) — shell commands atau
  executables
- Configurable via JSON: matcher + command pattern
- Hooks bisa **transform input** (UserPromptSubmit), **block tools**
  (PreToolUse), **auto-approve** (PermissionRequest)
...
Tambahan: `@import` processor untuk modular config, `save_memory` tool untuk
persist knowledge, MCP instructions injection, folder trust gating.
...
- **PolicyRules**: Per-tool `allow`/`deny`/`ask_user` dengan priority,
  `argsPattern` (regex), `toolAnnotations`, mode-scoping
- **SafetyCheckerRules**: External/in-process checkers (e.g., `allowed-path`,
  `conseca`)
...
2. **Developer observability** — Full DevTools server (selayaknya Chrome
   DevTools)
3. **Model routing** — Intelligent model selection per-turn
4. **Type safety** — TypeScript throughout with typed tool schemas
5. **Multi-consumer architecture** — Same core, multiple UIs (interactive, pipe,
   Zed)
