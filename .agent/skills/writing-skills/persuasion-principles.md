...
- Use tracking: update `<project-root>/docs/plans/task.md` for checklists
  (table-only tracker)
...
```markdown
✅ Checklists without `<project-root>/docs/plans/task.md` tracking = steps get
skipped. Every time. ❌ Some people find task tracking helpful for checklists.
```
